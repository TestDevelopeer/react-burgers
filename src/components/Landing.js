        ...
        <div className={'restaurant_select'}>
            <div className={'restaurant_select_top'}>
                <div onClick={displayList}
                     className={'restaurant_select_top-header font-effect-outline'}>{title ? title : 'Выберите ресторан'}</div>
                    ...
                    <ul>
                        {
                            restaurants.map(restaurant => {
                                return <li key={`restaurant_${restaurant.id}`} onClick={() => getTitle(restaurant)}>{restaurant.title}</li>;
                            })
                        }
                    </ul>
                </div>
